...
| Питання | проста українська | лише коли ваш запит можна зрозуміти двома способами і від цього залежить результат |
...
|---|---|---|
| `~/.claude/skills/{ccshut,shut}/` | копіюється плагін | Claude Code шукає плагіни лише тут; без цієї копії правило просто не вмикається |
| `~/.codex/skills/`, `~/.config/opencode/skills/`, `~/.gemini/skills/`, `~/.cursor/skills-cursor/`, `~/.agents/skills/` | копіюється плаский `SKILL.md` | у кожного агента свій шлях до скілів, спільного немає — тому по копії в кожен |
| `~/.codex/hooks.json` + `~/.codex/hooks/shut-*` | додаються два записи `SessionStart` | скіл вантажиться, лише коли агент сам вирішить, що той доречний, а правило про те, як говорити, потрібне вже з першого рядка. Хук подає його на старті кожної сесії. Чужі записи у файлі лишаються на місці |
| `~/.config/opencode/AGENTS.md`, `~/.gemini/GEMINI.md` | додається позначений блок | у цих двох агентів немає хука на старт сесії. Єдиний спосіб дати їм правило щоразу — покласти текст у файл, який вони читають самі |
| `~/.claude/settings.json` | `CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false`, `CLAUDE_CODE_ENABLE_AWAY_SUMMARY=0` | це підказки наступного запиту і авто-підсумок після паузи. Обидва додруковують текст від себе, ви за нього платите, і Shut його забороняє |
| `~/.claude/settings.local.json` | `outputStyle: Shut` | вбудований стиль відповіді може скинути правила стислості; цей стиль повторює їх, тому вони не губляться |
| `~/.claude/output-styles/shut.md` | встановлюється | файл самого стилю; без нього рядку вище нема на що вказувати |
| кожна знайдена проєктна тека `.claude/` | ті самі два налаштування | Claude читає налаштування проєкту поверх глобальних, тому проєкт може мовчки повернути те, що ви щойно вимкнули |
| ключ `language` у цих налаштуваннях | **видаляється** | будь-яке значення там додає «Always respond in \<lang\>» до всіх пояснень. Тоді англійська мітка кроку теж стає тією мовою, і поділ Shut ламається — хоч би яку мову там вказали |
| `~/.shut-backups/` | оригінали і стан | копії файлів до змін плюс список того, що саме змінено; `uninstall` читає звідси і повертає все як було |
...
Скрипт запускайте з тієї самої теки, куди клонували репо: `uninstall.cmd` і `uninstall.sh`
лежать саме там. Якщо теку вже видалили — склонуйте репо ще раз, цього досить: список змін
інсталятор пише в `~/.shut-backups/uninstall.json`, а не в теку репо, тому свіжий клон бачить
той самий стан.
